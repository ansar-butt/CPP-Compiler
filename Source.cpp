...
public:
	...
	{
		string a = "t";
		string b = std::to_string(tokenCount);
		tokenCount++;
		return a + b;
	}
	...
	void R()
	{
		...
		if (checkToken("LIT"))
		{
			...
			// addToSymbolTable("CHAR", tokenName);
			// fout2 << tokenName;
			fout2 << std::stoi(currLexeme.c_str());
			match("LIT");
		}
	...
	{
		// cout << "u" << endl;
		if (checkToken("INT") || checkToken("CHAR") || (checkToken("ID") && checkPeek("++")) || checkToken("ID") || checkToken("WHILE") || checkToken("IF") || checkToken("PRINT") || checkToken("PRINTLN") || checkToken("INPUT") || checkToken("COMMENT") || checkToken("/"))
		{
			prevNoOfTabs = noOfTabs;
			statements();
		}
...
public:
	...
	char *getSymbolAdress(string symbol)
	{
		...
		{
			string arr = std::to_string(symbol[1]);
			char *result = new char[arr.size() + 1];
			strcpy(result, arr.c_str());
			return result;
		}
	...
	bool isAssignment(vector<string> tokens)
	{
		if (tokens.size() == 3 && tokens[1] == "=")
		{
			...
			data.push_back(std::stoi(getSymbolAdress(tokens[2])));
			data.push_back(std::stoi(getSymbolAdress(tokens[0])));
	...
	bool isOutput(vector<string> tokens)
	{
		if (tokens.size() == 2 && tokens[0] == "OUT")
		{
			...
			if (hasEnding(tokens[1], "n"))
			{
				...
				data.push_back(std::stoi(getSymbolAdress(tokens[1].substr(0, tokens[1].length() - 2))));
				data.push_back(1);
				addToMemArrayAndFile(data);
			}
			else
			{
				...
				data.push_back(std::stoi(getSymbolAdress(tokens[1])));
				data.push_back(0);
				addToMemArrayAndFile(data);
			}
		}
	...
	bool isIf(vector<string> tokens)
	{
		if (tokens.size() == 6 && tokens[0] == "if")
		{
			...
			data.push_back(std::stoi(getSymbolAdress(tokens[1])));
			data.push_back(std::stoi(getSymbolAdress(tokens[3])));
			data.push_back(std::stoi((char *)tokens[5].c_str()));
			addToMemArrayAndFile(data);
		}
		return false;
	}
	bool isGoto(vector<string> tokens)
	{
		if (tokens.size() == 2 && tokens[0] == "goto")
		{
			...
			data.push_back(std::stoi((char *)tokens[1].c_str()));
			addToMemArrayAndFile(data);
		}
		return false;
	}
	...
	bool isArithmetic(vector<string> tokens)
	{
		if (tokens.size() == 5 && tokens[1] == "=")
		{
			...
			data.push_back(std::stoi(getSymbolAdress(tokens[2])));
			data.push_back(std::stoi(getSymbolAdress(tokens[4])));
			data.push_back(std::stoi(getSymbolAdress(tokens[0])));
			addToMemArrayAndFile(data);
		}
		return false;
	...
	void initializeBytes()
	{
		...
		while (!fin.eof())
		{
			...
			if (temp.size() > 1)
			{
				...
				int address = std::stoi(addressTemp);
